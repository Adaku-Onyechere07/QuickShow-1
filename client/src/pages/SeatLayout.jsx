...
const SeatLayout = () => {
  ...
  return show ? (
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
        {/* Available timings */}
        <div className='w-60 bg-primary/10 border border-primray/20 rounded-lg py-10 h-max md:sticky md:top-30'>
        ...
        <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
            ...
            <button onClick={()=> navigate('/my-bookings')} className='flex items-center gap-1 mt-20 px-10 py-3 text-sm bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer active:scale-95'>
                Proceed <ArrowRight strokeWidth={3} className='w-4 h-4' />
            </button>
        </div>
    </div>
  ) : (
